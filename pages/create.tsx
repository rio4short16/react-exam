...
const CreateInvoicePage = () => {
  ...
  const onSubmitHandler = async (values: any) => {
    ...
    try {
      const newInvoice: InvoiceType = {
        ...
        totalPrice:
          Number(values?.productPrice) * Number(values?.orderQuantity),
        productPrice: Number(values?.productPrice),
        orderQuantity: Number(values?.orderQuantity),
      };

      const listOfInvoices = [...invoices, newInvoice];
      setInvoices(listOfInvoices);
      router?.push("/");
    } catch (error) {
      showError("Something went wrong while saving this invoice!");
    }
  };
  ...
  return (
    <React.Fragment>
      <HomeLayout title="Create Invoice">
